    ...
    class Process
    {
        ...
        public void createPolynom(int stepen)
        {
            try
            {
                ...
                //sw.WriteLine();
                sw.WriteLine();
                sw.WriteLine("Complete polynomial:");
                //sw.Write("0+");
        ...
        public double CheckError()
        {
            ...
            double absolutSKO = Math.Sqrt(Sum_abs / kol);
            return absolutSKO;
        }
        ...
        public void regression_func( double[][] Mas)
        {
			try
			{
				...
				if (ObjectsList[0].X_parameters.Count > 2)
				{
					...
					if (Er3 == Er2)
					{
						TempTypeOfRegress.Add("Alg.2/Alg.3");
						// Checking(Global.SOLE, regressCoefTEMP3);
						regressCoef = LinearRegressionProgram.Solve(Mas, 3);
						//CopyToRegresList(regressCoefTEMP3);
						//List<string> ListList = new List<string>(this.listOfRegres);
				...
				else
				{
					...
					if (Er3 == Er1 && Er3 == Er2)
					{
						TempTypeOfRegress.Add("Alg.1/Alg.2/Alg.3");
						Checking(Global.SOLE, regressCoefTEMP2);
						regressCoef = LinearRegressionProgram.Solve(Mas, 2);
						//CopyToRegresList(regressCoefTEMP3);
						//List<string> ListList = new List<string>(this.listOfRegres);
					...
					if (Er3 == Er1 && Er3 < Er2)
					{
						TempTypeOfRegress.Add("Alg.1/Alg.3");
						Checking(Global.SOLE, regressCoefTEMP3);
						regressCoef = LinearRegressionProgram.Solve(Mas, 3);
						//CopyToRegresList(regressCoefTEMP3);
						//List<string> ListList = new List<string>(this.listOfRegres);
      ...
      where M : ObjectGroup
        ...
        public void commonError()
        {
            for (int i = 0; i < groupList.Count;i++ )
            {
                ...
                {
                    if (groupList[i].polynomeView==groupList[j].polynomeView)
                    {
            ...
            for (int i = 0; i < groupList.Count; i++)
            {
                if (Double.IsNaN(groupList[i].commonAbsEror))
                {
                    groupList.Remove(groupList[i]);
                    i--;
                }
                ...
                for (int i = 0; i < groupList.Count; i++)
                {
                    ...
                    StreamWriter sw;
                    sw = File.AppendText("Test.txt");
                    sw.WriteLine("Polynomial combination № {0}", i + 1);
                    sw.WriteLine("Created polynomial info:");
                    sw.WriteLine(groupList[i].polynomeView);
        ...
        */
        ...
        private void stringSeparate(string str, int num)
        {
            Numbers.Clear();
        ...
        {            
            ...
            {
                stringSeparate(combination[index], Global.combinationSize);//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        public void polynomeResizeForEGA (string index, int NumbCount) //index - это индекс элемента в списке поколения(популяции)
        {
            polynome_string = "";
            Array.Resize(ref polynomString, NumbCount /*stringList[index].Count*/);

            List<string> temparray = new List<string>();

            for (int i = 0; i < NumbCount /*stringList[index].Count*/; i++)
            {
                stringSeparate(index, NumbCount);//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                temparray.Add(temppolynome[Numbers[i] - 1]/*Convert.ToInt32(stringList[index][i])-1]*/);
            }
            for (int i = 0; i < temparray.Count; i++)
            {
        ...
        public void PutPolynomeElementsToList()
        {
            for (int i=0;i<groupList.Count;i++)
            {
                ...
                //if (groupList[i].polynomeView.Length == 1)
                //{
                ...
                //if (groupList[i].polynomeView.Length == 1)
                //{
                //    groupList[i].PolynomeMembers.Add(groupList[i].polynomeView);
                //}

                //var include = groupList[i].polynomeView.IndexOf('0');

                if (groupList[i].polynomeView.IndexOf('+') == -1)
                {
                    groupList[i].PolynomeMembers.Add(groupList[i].polynomeView);
                }
